...
namespace server
{
	...
	abstract class Room
	{
		...
		*/
		...
		/**
		 * Sends a message to one specific member in the room.
		 */

        protected void sendToOne(ASerializable pMessage, TcpMessageChannel pChannel )
        {
            pChannel.SendMessage(pMessage);
        }
